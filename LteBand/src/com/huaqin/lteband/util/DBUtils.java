...
public class DBUtils {
     ...
     * @param asyncQuery
     */
    private static void syncWithDB(QueryListener listener, SimStateInfo todoInfo, AsyncQueryHandler asyncQuery,
            int operatorCode) {
        String idColumnName = BandColumn.ID;
        ContentValues values = todoInfo.makeContentValues();
        String selection;

        switch (operatorCode) {
        case DBUtils.OPERATOR_UPDATE:
            selection = idColumnName + "=" + todoInfo.getId();
            asyncQuery.startUpdate(0, listener, TodoAsyncQuery.TODO_URI, values, selection, null);
            break;

        case DBUtils.OPERATOR_INSERT:
            asyncQuery.startInsert(0, listener, TodoAsyncQuery.TODO_URI, values);
            LogUtils.d(TAG, "DBUtils.OPERATOR_INSERT URI="+TodoAsyncQuery.TODO_URI);
            break;

        case DBUtils.OPERATOR_DELETE:
            selection = idColumnName + "=" + todoInfo.getId();
            asyncQuery.startDelete(0, listener, TodoAsyncQuery.TODO_URI, selection, null);
            break;
        case DBUtils.OPERATOR_NONE:
     ...
     * Write data to DB.
     */
    public static void writeAdapterDataToDB(SimStateInfo data, AsyncQueryHandler asyncQuery,
            int operatorCode) {
     ...
     * Write data to DB.
     */
    public static void writeAdapterDataToDB(QueryListener listener,SimStateInfo data, AsyncQueryHandler asyncQuery,
            int operatorCode) {
     ...
     * @return
     */
    public static String getDateText(Context context, long dateMillis) {
        String timeFormat = null;
        boolean is24HourMode = get24HourMode(context);


        //if("en".equals(Locale.getDefault().getLanguage()) && !is24HourMode ){

            Date date = new Date(dateMillis);
            String date_display= new SimpleDateFormat("yyyy-MM-dd hh:mm:ss a").format( date);
            return date_display;
    }
